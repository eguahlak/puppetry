...
interpolate start end index =
    let
        delta =
            toFloat (start.index - index) / toFloat (start.index - end.index)
    in
    Puppetry.Color.interpolate delta start.color end.color
